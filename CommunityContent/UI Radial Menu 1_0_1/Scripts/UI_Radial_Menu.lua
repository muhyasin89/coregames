...
local function open_radial_menu()
	...
	if(#items > 0) then
		...
		for i = 1, #items do
			...
			tween:on_complete(function()
				...
				local fade_tween = YOOTIL.Tween:new(.25, { a = 0 }, { a = 1 })
				local icon_col = icon:GetColor()

				fade_tween:on_change(function(c)
					icon_col.a = c.a
					icon:SetColor(icon_col)
				end)

				fade_tween:on_complete(function()
					fade_tween = nil
				end)

				table.insert(tweens, #tweens + 1, fade_tween)
			end)
...
local function close_radial_menu()
	tweens = {}
	clear_buttons()
	is_open = false
...
local function disable_radial_menu()
	can_open = false
end

local function enable_radial_menu()
	can_open = true
end
