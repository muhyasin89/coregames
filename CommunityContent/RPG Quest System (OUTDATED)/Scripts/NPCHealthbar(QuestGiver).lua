...
function GetQuest()
	if ROOT:GetCustomProperty("HasQuest") == true then
		...
		local questRes = player:GetResource(questTable[NPCQuest][RES_NAME])
		if questRes == 1 then
			return ""
		elseif questRes == 0 and RequiresQuestComplete and player:GetResource(questTable[QuestCompleteId][RES_NAME]) < 100000 then
			return "!*"
		elseif questRes == 0 and questTable[NPCQuest][reqLevel] > player:GetResource("level") then
			return "!*"
