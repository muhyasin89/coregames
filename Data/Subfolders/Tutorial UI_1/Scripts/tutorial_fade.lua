...
print(parent:IsVisibleInHierarchy())
function fadeInUI()
	-- print(parent)
	...
	for k,v in pairs(objects) do
		...
		if string.sub(v.name, -5) ~= "Panel" then
			--[[if pcall(obj:GetColor()) == false then 
				error("got nil in obj")
			else]]--
				objC = obj:GetColor()
				print(objC)
...
function CheckVisibleCursor()
	local isShowCursor = UI.IsCursorVisible()
	if(isShowCursor == false) then
